...
const FabricDc = () => {
  ...
  const handleView = async (record) => {
    ...
    if (selectedInward) {
      ...
      const existingDcs = allDcs.filter(
        (dc) => dc.inwardNo === record.inwardNo && dc.id !== record.id,
      );
      const existingReturns = allReturns.filter(
        (ret) => ret.inwardNo === record.inwardNo,
      );
      ...
      setPendingInward(
        Number(selectedInward.totalQty) - usedInDc - usedInReturn,
      );
    ...
    setPrintData({
      ...
      recWeight: Number(
        inwards.find((i) => i.grnNo === record.inwardNo)?.totalQty || 0,
      ).toFixed(3),
    ...
    setDetails(
      record.details?.map((d) => {
        return {
          ...
          lossPercentage:
            (d.processWeight || 0) > 0
              ? (((d.processWeight || 0) - (d.dcWeight || 0)) /
                  (d.processWeight || 0)) *
                100
              : 0,
    ...
    setSelectedProcesses(
      record.processes?.map((p) => ({ ...p, key: p.id })) || [],
    );
  ...
  const handleSubmit = async (shouldPrint = false) => {
    try {
      ...
      const hasInvalidProcessWeight = details.some(
        (d) => (Number(d.processWeight) || 0) <= 0,
      );
      ...
      const hasValidWeight = details.some(
        (d) =>
      ...
      const duplicate = allDcs.find(
        (f) => f.dcNo?.trim() === trimmedDcNo && f.id !== editingId,
      );
      if (duplicate) {
      ...
      const inwardResponse = await getFabricInwards("", 1, 1000);
      const allInwards = inwardResponse.data || [];
      const duplicateInward = allInwards.find(
        (i) => i.grnNo?.trim() === trimmedDcNo,
      );
      if (duplicateInward) {
      ...
      const data = {
        ...
        details: details.map((d) => ({
          ...
          processes:
            enableItemWiseProcess && d.processes
  ...
  const handleInwardSelect = async (inwardNo) => {
    ...
    if (selectedInward) {
      ...
      const existingDcs = allDcs.filter(
        (dc) => dc.inwardNo === inwardNo && dc.id !== editingId,
      );
      const existingReturns = allReturns.filter(
      ...
      if (selectedInward.details && selectedInward.details.length > 0) {
        ...
        const loadedDetails = selectedInward.details.map((d, idx) => {
          ...
          const lossPercentage =
            processWt > 0 ? ((processWt - dcWt) / processWt) * 100 : 0;
        ...
        setDetails(
          loadedDetails.filter((d) => d.processWeight > 0 || d.rolls > 0),
        );
      }
  ...
  const handleInwardDetailSelect = async (key, inwardDetailId) => {
    ...
    if (selectedDetail) {
      ...
      const existingDcs = allDcs.filter(
        (dc) => dc.inwardNo === currentInwardNo && dc.id !== editingId,
      );
      const existingReturns = allReturns.filter(
        (ret) => ret.inwardNo === currentInwardNo,
      );
      ...
      setDetails((currentDetails) => {
        const usedWeightInForm = currentDetails
          .filter(
            (d) => d.key !== key && d.inwardDetailId === selectedDetail.id,
          )
      ...
      details.map((d) => {
        if (d.key === key) {
          ...
          if (field === "processWeight" || field === "dcWeight") {
            ...
            const processWeight =
              field === "processWeight" ? value : d.processWeight;
            const dcWeight =
              field === "dcWeight"
                ? value
                : field === "processWeight"
            ...
            updated.lossPercentage =
              processWeight > 0
                ? ((processWeight - dcWeight) / processWeight) * 100
                : 0;

            console.log("Result:", {
              weightLoss: updated.weightLoss,
              lossPercentage: updated.lossPercentage,
            });
          }
    ...
    { title: "Sl.No", width: 50, render: (_, record, index) => index + 1 },
    {
      ...
      render: (_, record) => {
        if (isViewMode && record.inwardDetailId) {
          const selectedDetail = inwardDetails.find(
            (d) => d.id === record.inwardDetailId,
          );
          if (selectedDetail) {
            const dia =
              dias.find((dia) => dia.id === selectedDetail.diaId)?.masterName ||
              "";
            const fabric =
              fabrics.find((f) => f.id === selectedDetail.fabricId)
                ?.masterName || "";
            const color =
              colors.find((c) => c.id === selectedDetail.colorId)?.masterName ||
              "";
            const gsm = selectedDetail.gsm || "";
            const designInfo = selectedDetail.designNo
              ? ` | ${selectedDetail.designNo}`
              : "";
            const designName = selectedDetail.designName
              ? ` | ${selectedDetail.designName}`
              : "";
            const colorCount = selectedDetail.noOfColor
              ? ` | ${selectedDetail.noOfColor}`
              : "";
        ...
        return (
          ...
          >
            {inwardDetails.map((d) => {
              const dia =
                dias.find((dia) => dia.id === d.diaId)?.masterName || "";
              const fabric =
                fabrics.find((f) => f.id === d.fabricId)?.masterName || "";
              const color =
                colors.find((c) => c.id === d.colorId)?.masterName || "";
              const gsm = d.gsm || "";
      ...
      : []),
    {
      ...
      render: (val, record) => (
        <InputNumber
          disabled={isViewMode}
          value={val}
          onChange={(v) => handleDetailChange(record.key, "processWeight", v)}
          style={{ width: "100%" }}
          precision={3}
          autoComplete="off"
        />
    ...
    },
    {
      ...
      render: (val, record) => (
        <InputNumber
          disabled={isViewMode}
          value={val}
          onChange={(v) => handleDetailChange(record.key, "dcWeight", v)}
          style={{ width: "100%" }}
          precision={3}
          autoComplete="off"
        />
  ...
  const totalProcessWeight = details.reduce(
    (sum, d) => sum + (Number(d.processWeight) || 0),
    0,
  );
  const totalRolls = details.reduce((sum, d) => sum + (d.rolls || 0), 0);
  ...
  useEffect(() => {
    const updateRemaining = async () => {
      if (inwardQty > 0 && form.getFieldValue("grnNo")) {
        ...
        existingDcs.forEach((dc) => {
          if (dc.details && dc.details.length > 0) {
            usedProcessWeight += dc.details.reduce(
              (sum, detail) => sum + (Number(detail.processWeight) || 0),
              0,
            );
          }
        });
        ...
        const remaining =
          inwardQty - usedProcessWeight - usedInReturn - totalProcessWeight;
  ...
  return (
    <Card>
      ...
      <div style={{ display: "none" }}>
        {printData && <FabricDCPrint ref={printRef} data={printData} />}
      </div>
    </Card>
  );
};
