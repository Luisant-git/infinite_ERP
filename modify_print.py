...
path = "d:/infinite_ERP/frontend/src/components/prints/FabricBillPrint.jsx"
...
for i in range(len(lines)-1, -1, -1):
    if line.strip() == ");":
        # Wait, the above loop is wrong. `if lines[i].strip() == ");":`
        pass
...
original_return = lines[start_idx+1:end_idx]
indented_original = []
for line in original_return:
    if line.strip():
        indented_original.append("        " + line)
    else:
        indented_original.append(line)

new_return = """    const itemsPerPageDC = 8;
...
new_lines = lines[:start_idx] + [new_return] + lines[end_idx+1:]
